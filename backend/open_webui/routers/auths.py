    ...
    *,
    ...
    try:
        if bot is not None and TG_CHAT_ID:
            log.info(
                f"Sending Telegram message for pending user registration: {name}, {email.lower()}"
            )
            if role == "pending":
                await bot.send_message(
                    TG_CHAT_ID,
                    f"Користувач {name} надіслав запит на реєстрацію аккаунта. Почта {email.lower()}. Посилання на адмін панель з користувачами https://gpt.ta-da.ua/admin/users .",
                )
            if role == "user":
                await bot.send_message(
                    TG_CHAT_ID,
                    f"Користувач {name} зареєструвався. Почта {email.lower()}. Посилання на адмін панель з користувачами https://gpt.ta-da.ua/admin/users .",
                )
    except Exception as e:
        log.info(f"Error {e}")
